...
fun MDLCreator.textField(floatingLabel: Boolean = false, expandable: Boolean = false, disabled: Boolean = false, isInvalid: Boolean = false): MDLTextFieldElement =
        MDLTextFieldElement(div(attr.classes("mdl-textfield", "mdl-js-textfield")
...
class MDLTextFieldElement internal constructor(val e: Element) : Element(e) {
    fun input(type: InputType? = null, pattern: String? = null, attributes: MutableMap<String, Any> = attr)
            = e.insert().input(type, attributes = attributes.classes("mdl-textfield__input").set("pattern", pattern))

    fun label(forInput: InputElement, attributes: MutableMap<String, Any> = attr)
            = e.insert().label(attributes
    ...
    fun error(attributes: MutableMap<String, Any> = attr) = e.insert().span(attributes.classes("mdl-textfield__error"))
