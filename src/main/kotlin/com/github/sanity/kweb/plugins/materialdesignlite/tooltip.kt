...
fun MDLElement.tooltip(tag: TooltipTag = TooltipTag.div,
                       large: Boolean = false, position: TooltipPosition? = null): Element {
    kotlin.require(this.id != null) { "id must be specified to add a tooltip to an element" }
    return createElement(tag.name, attr
            .classes("mdl-tooltip")
            .classes(onlyIf = large, classes = "mdl-tooltip--large")
            .classes(onlyIf = position != null, classes = "mdl-tooltip--${position!!}")
            .set("data-mdl-for", this.id))
}
